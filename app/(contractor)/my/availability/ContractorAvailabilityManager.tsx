...
import { contractorAddAvailability, contractorDeleteAvailability } from "../../actions";
...
export function ContractorAvailabilityManager({ windows }: { windows: Window[] }) {
  ...
  const onSubmit = (values: AvailabilityInput) => {
    ...
    startTransition(async () => {
      const res = await contractorAddAvailability(values);
      if (!res.ok) {
  ...
  const onDelete = (id: string) => {
    startTransition(async () => {
      await contractorDeleteAvailability(id);
      router.refresh();
    });
  };
